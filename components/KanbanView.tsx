...
const EjeCard = memo(function EjeCard({ p, onSelect, onToggleFoco, canEditFoco }: {
  ...
  return (
    <button
      onClick={() => onSelect(p)}
      className={`w-full text-left border rounded-xl p-3 hover:shadow-md transition-[box-shadow,border-color,background-color] duration-150 ease-[cubic-bezier(0.25,1,0.5,1)] group relative ${
    ...
    >
      {canEditFoco ? (
        <button
          ...
          className={`absolute top-2 right-2 p-1 rounded transition-colors duration-150 ease-[cubic-bezier(0.25,1,0.5,1)] ${
...
const MinistryRow = memo(function MinistryRow({ p, onSelect, onToggleFoco, canEditFoco }: {
  ...
  return (
    <button
      onClick={() => onSelect(p)}
      className={`w-full text-left px-3 py-2.5 border rounded-xl hover:shadow-sm transition-[box-shadow,border-color,background-color] duration-150 ease-[cubic-bezier(0.25,1,0.5,1)] flex items-center gap-3 ${
    ...
    >
      ...
      {canEditFoco ? (
        <button
          onClick={(e) => { e.stopPropagation(); onToggleFoco(p.n, p.id, !enFoco) }}
          className={`flex-shrink-0 p-1 -m-1 rounded transition-colors duration-150 ease-[cubic-bezier(0.25,1,0.5,1)] ${
            enFoco ? 'text-amber-500 hover:text-amber-700' : 'text-gray-300 hover:text-amber-400'
          }`}
          title={enFoco ? 'Quitar del foco' : 'Marcar en foco'}
        >

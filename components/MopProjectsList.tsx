...
import { MOP_PIPELINE, matchMopMeta, pipelineOrder } from '@/lib/portfolioMeta'
import ProcessDrawer, { ColumnHeaderTooltip } from './PortfolioProcessSidebar'

const MOP_INTRO =
  'Las iniciativas de inversión pública del MOP siguen el ciclo del Sistema Nacional de Inversiones (SNI). Pasan por estudios crecientemente detallados — perfil, prefactibilidad y factibilidad — antes de aprobarse para diseño y ejecución. Una vez construidas, entran a operación o quedan bajo conservación/mantención para preservar el nivel de servicio.'
...
function fmtInversion(miles: number | null): string {
  ...
  const { value, unit } = fmtInversionTotal(miles)
  return `${value} ${unit}`
}

// KPI: el field `inversion_miles` viene del HTML MOP en miles de CLP. Convertimos
// a millones de CLP (÷1.000) y elegimos la escala chilena: millones (10^6) →
// mil millones (10^9) → billones (10^12).
function fmtInversionTotal(miles: number): { value: string; unit: string } {
  const mmCLP = miles / 1_000
  if (mmCLP >= 1_000_000) return {
    value: `$ ${(mmCLP / 1_000_000).toLocaleString('es-CL', { maximumFractionDigits: 2 })}`,
    unit:  'billones CLP',
  }
  if (mmCLP >= 1_000) return {
    value: `$ ${(mmCLP / 1_000).toLocaleString('es-CL', { maximumFractionDigits: 1 })}`,
    unit:  'mil millones CLP',
  }
  return {
    value: `$ ${mmCLP.toLocaleString('es-CL', { maximumFractionDigits: 0 })}`,
    unit:  'millones CLP',
  }
}
...
function KpiTile({ label, value, unit, accent }: { label: string; value: string; unit?: string; accent?: 'green' | 'blue' }) {
...
// Card amplia para columna kanban (sin chip de etapa — vive en el header).
// Muestra nombre, servicio, comunas, programa/financiamiento e inversión.
function MopKanbanCard({ proyecto, onSelect }: { proyecto: MopProject; onSelect: (p: MopProject) => void }) {
  return (
    <button
    ...
    >
      ...
      {proyecto.comunas && (
        <p className="text-[10px] text-gray-500 mt-1 line-clamp-1" title={proyecto.comunas}>
          {proyecto.comunas}
        </p>
      )}
...
// Vista tipo "Por eje" de Kanban: una columna por etapa, ordenadas según el
// pipeline canónico SNI/MOP (Idea → Operación). A la derecha, sidebar con
// el proceso completo y tooltips sobre cada header.
function MopKanban({ proyectos, onSelect }: { proyectos: MopProject[]; onSelect: (p: MopProject) => void }) {
  const columns = useMemo(() => {
    const groups: Record<string, { items: MopProject[]; meta: ReturnType<typeof matchMopMeta> }> = {}
    for (const p of proyectos) {
      const key = p.etapa ?? 'Sin etapa'
      if (!groups[key]) groups[key] = { items: [], meta: matchMopMeta(p.etapa) }
      groups[key].items.push(p)
    }
    return Object.entries(groups)
      .map(([etapa, { items, meta }]) => ({ etapa, items, meta }))
  ...
  return (
    <div className="flex gap-3 h-[calc(100vh-340px)] min-h-[480px] -mx-1 px-1">
      <div className="flex-1 overflow-x-auto overflow-y-hidden">
        ...
        >
          {columns.map(({ etapa, items, meta }) => {
            const c = etapaBadge(etapa)
            const displayName = meta?.canonical ?? etapa
            return (
              <div key={etapa} className="flex flex-col overflow-hidden">
                <div className={`relative group flex items-center gap-2 px-2.5 py-2 rounded-lg mb-2 ${c.bg} ${meta ? 'cursor-help' : ''}`}>
                  <span className={`text-[11px] font-semibold leading-tight ${c.text} flex-1 truncate`} title={etapa}>
                    {displayName}
                  </span>
                ...
                <div className="flex-1 overflow-y-auto space-y-1.5 pr-1">
                  {items.map(p => (
                    <MopKanbanCard key={p.cod_p} proyecto={p} onSelect={onSelect} />
                  ))}
                </div>
              </div>
            )
      ...
      <ProcessDrawer title="Proceso MOP" intro={MOP_INTRO} pipeline={MOP_PIPELINE} />
    </div>
  )
}
...
export default function MopProjectsList({ proyectos, total, loading, error }: Props) {
  ...
  // Catálogo para filtros (top 8 por frecuencia).
  // El filtro por etapa se removió: ahora "etapa" es la columna del kanban.
  const serviciosDisponibles = useMemo(() => {
    const sCount: Record<string, number> = {}
    for (const p of proyectos) {
      if (p.servicio) sCount[p.servicio] = (sCount[p.servicio] ?? 0) + 1
    }
    return Object.entries(sCount).sort((a, b) => b[1] - a[1]).slice(0, 8).map(e => e[0])
  }, [proyectos])
  ...
  }, [proyectos, search, filterServicio, comunaSearch, sortBy])
  ...
  const filtersActive = search !== '' || filterServicio.size > 0 || comunaSearch !== ''
  function clearFilters() {
    setSearch(''); setFilterServicio(new Set()); setComunaSearch('')
  }
    ...
    <div className="relative">
      ...
      <div className="px-5 pb-2 space-y-2">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <input
            ...
            placeholder="Comuna..."
            className="w-24 px-2 py-1 text-[11px] text-gray-900 placeholder:text-gray-500 border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-slate-300 bg-white"
          />
      ...
      <div className="px-5 pb-1">
        ...
        {filtered.length === 0 ? (
          ...
          <MopKanban proyectos={filtered} onSelect={setSelected} />
        )}
      </div>
      <div className="pb-3" />
    </div>
